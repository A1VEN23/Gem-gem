...
// Dust threshold: values below this are treated as 0 for display
const DUST_THRESHOLD = 1e-7;

function fmtBal(num, sym) {
  const n = parseFloat(num);
  if (isNaN(n) || n === 0 || Math.abs(n) < DUST_THRESHOLD) return `0 ${sym}`;
        ...
        {assets.map((asset, index) => {
          ...
          return (
            <div key={asset.id} className="tap-row"
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ color: realBal < DUST_THRESHOLD ? "#555" : "white", fontWeight: 600, fontSize: 15 }}>
                  {settings?.hideBalance ? "•••" : balStr}
                </div>
                {realBal >= DUST_THRESHOLD && !settings?.hideBalance
